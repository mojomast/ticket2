...
app.get('/articles/:id', async (c) => {
  const article = await kbService.getArticle(c.req.param('id'));
  return c.json({ data: article, error: null });
});

// GET /api/kb/articles/by-slug/:slug
app.get('/articles/by-slug/:slug', async (c) => {
  const article = await kbService.getArticleBySlug(c.req.param('slug'));
  return c.json({ data: article, error: null });
});

// PATCH /api/kb/articles/:id
app.patch('/articles/:id', validateBody(updateArticleSchema), async (c) => {
  const session = c.get('session');
  const data = c.get('body') as any;
...
// POST /api/kb/links
app.post('/links', validateBody(createLinkSchema), async (c) => {
  const session = c.get('session');
  const data = c.get('body') as any;
...
app.delete('/links/:id', async (c) => {
  await kbService.unlinkArticle(c.req.param('id'));
  return c.json({ data: { success: true }, error: null });
});
